...
async function run() {
  ...
  await client.query("ALTER TABLE accounts ADD COLUMN IF NOT EXISTS card_number VARCHAR(20) UNIQUE");
  
  // Populate existing accounts with mock card numbers
  ...
  console.log('Database updated with card numbers');
  await client.end();
}
